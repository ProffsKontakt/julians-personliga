...
> **Version note (verified against konsta@5.4.0 + tailwindcss@4).**
> Konsta UI v5 requires **Tailwind CSS v4** and is configured **CSS-first**.
> There is no `tailwind.config.js` and no `konsta/config` export any more — both
> belonged to Konsta v3/v4 on Tailwind v3. Always follow the v5 setup below and
> https://konstaui.com/react/installation over any older snippet.

### Installation

```bash
# React / Vue / Svelte — one package for all three
npm install konsta

# Required: Tailwind CSS v4
npm install -D tailwindcss @tailwindcss/vite   # Vite
# or, for Next.js / PostCSS pipelines:
npm install -D tailwindcss @tailwindcss/postcss
```

### Tailwind v4 configuration (CSS-first)

Konsta v5 ships its theme as a CSS file you import after Tailwind. No JS config file.
...
/* Tell Tailwind v4 to scan Konsta's own components for classes */
@source '../node_modules/konsta/react';   /* or /vue, /svelte */

/* Brand color — drives every --k-color-* token (iOS + Material ramps) */
@theme {
  --color-brand-primary: #007aff;
}
```

Framework-specific theme entry points also exist and are equivalent:
`konsta/react/theme.css`, `konsta/vue/theme.css`, `konsta/svelte/theme.css`.
...
Konsta v5 ships Apple's iOS 26 Liquid Glass material natively — this is the
preferred way to get glass in a Konsta app, not a third-party glass plugin.

```tsx
import { App, Page, Navbar, Glass, Fab } from 'konsta/react';

<App theme="ios">
  <Page>
    {/* Translucent, blurred, specular-highlighted chrome */}
    <Navbar title="Dashboard" translucent />

    {/* Raw glass surface for custom content */}
    <Glass className="m-4 rounded-3xl p-4">
      <p>Content on a real Liquid Glass surface.</p>
    </Glass>
  </Page>
</App>
```
...
|------|---------|---------|
| `highlight` | `true` | iOS liquid-glass specular highlight that tracks the pointer |
| `colors.bgIos` | `bg-ios-light-glass dark:bg-ios-dark-glass` | Glass tint |
| `colors.shadowIos` | `shadow-ios-light-glass dark:shadow-ios-dark-glass` | Edge/inner-bevel shadow stack |

Theme tokens available to your own CSS: `--color-ios-light-glass`,
`--color-ios-dark-glass`, `--shadow-ios-light-glass`, `--shadow-ios-dark-glass`,
plus `-thumb` and `-fab` variants.

**Rule of thumb:** glass belongs to the navigation/control layer (navbar, tabbar,
toolbar, sheets, FABs), not to the content layer. Never stack one glass surface
directly on another — the blur compounds and legibility dies.
...
### Custom Colors with Tailwind v4

Konsta v5 derives its whole color system from `--color-brand-primary`. Set it in
`@theme` and the plugin generates the matching iOS and Material light/dark ramps
(`--k-color-*`), which the components consume automatically.

```css
/* globals.css — after @import 'konsta/theme.css' */
@theme {
  --color-brand-primary: #6366f1;

  /* Extra named brands become .k-color-brand-<name> scope classes */
...
```tsx
/* Scope a subtree to a different brand color */
<div className="k-color-brand-green">
  <Button>Green primary in here</Button>
</div>
```
